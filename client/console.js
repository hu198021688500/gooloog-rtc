...
function connectNS(nsData) {
	...
	socketNS.on("connect", function() {
		socketNS.emit("init_user", {GID : myGID});
...
function connectSS(ssData) {
	...
	sockets[toGID].on("connect", function() {
		sockets[toGID].emit("init_user", {GID : myGID});
		sockets[toGID].on("send_msg_ok", function(sendMsgOkData) {
		...
		sockets[toGID].on("revice_msg", function(reviceMsgData) {
			console.log("recive message form " + reviceMsgData.FGID + " " + reviceMsgData.sn + ":" + reviceMsgData.msg);
