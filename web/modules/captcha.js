/* 最多支持99个,后缀名是png */
var captchapath = _rrest.config.baseDir + '/view/captcha_pic/', captcha_ary = [
		{
			id : 1,
			url : captchapath,
			value : '东洋砍刀'
...
var c_a = captcha_ary, mongo = _rrest.mongo, // 连接mongo数据库方法
get_id = require('./stools').get_id, captcha = {
...
captcha.generate = function(id) {// 生成验证码方法
	...
	var head = 10 + Math.ceil(Math.random() * 89), cutpo = head.toString()
			.slice(-1);
	var front = ts.slice(0, cutpo), back = ts.slice(cutpo);
	res = head + front + id + back;
	return res;
};
captcha.decode = function(str) {// 验证码解码
	var head = str.slice(1, 2), str = str.slice(2), tsfront = str.slice(0,
			parseInt(head, 10)), id = str.slice(parseInt(head, 10), parseInt(
					head, 10)
					+ 2), intid = parseInt(id, 10), tsback = str
			.slice(parseInt(head) + 2), ts = tsfront + tsback, now = (new Date())
			.getTime();
	if (now - ts > 1000 * 15)
		return false;
	if (intid != id)
...
captcha.mongodbcreat = function(u_obj, cb) {// 去mongodb数据库创建验证码
	var ary = u_obj.ca_ary, po = u_obj.po;
							...
							collection.findOne({
										...
										if (err)
											return restlog
													.err('对比验证码失败：' + err);
										if (doc && doc.po == po)
											res = true; // 如果对比成功，返回true，否则为false
										captcha.destory('', id);
										cb(res);
									});// findOne
						});// collection
...
captcha.intial = function() { // 初始化，异步脚本删除验证码
	process.nextTick(function() {
				mongo(function(err, db, release) { // 建立集合
							if (err)
								return release();
							db.createCollection(captcha.dbcolname, function(
											err, col) {
										release();
										if (err)
											restlog.error('创建验证码集合失败：' + err);
									});
						});
